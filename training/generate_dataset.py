...
if data_args.valid_pct_range:
    ...
    dataset = dataset.select(ranges[int(start_valid_pct*len(ranges)*0.01):int(end_valid_pct*len(ranges)*0.01)])
    ...
    dataset = Dataset.from_pandas(pd.read_json(data_args.raw_data))
    ranges = list(range(len(dataset)))
    dataset = dataset.select(ranges[int(start_train_pct*len(ranges)*0.01):int(end_train_pct*len(ranges)*0.01)])
...
tokenizer = AutoTokenizer.from_pretrained(
    model_args.model_name_or_path, max_length=token_max_length, padding="max_length", add_special_tokens=True
)
...
dataset_gen_func = DatasetGenerate(
    dataset, model, tokenizer, decoding_options, [data_args.context_col, data_args.to_predict_col, data_args.marker_col], replace_one=None
)
